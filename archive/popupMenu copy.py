#popupMenu.py
from PyQt5.QtWidgets import QMenu, QAction, QInputDialog, QCalendarWidget, QDialog, QVBoxLayout, QPushButton, QLabel, QWidgetAction, QTimeEdit
...
class NodePopupMenu(QMenu):
    def __init__(self, parent=None):
        ...
        # 시간 설정
        self.action_time = QAction("시간 설정", self)
        self.action_time.triggered.connect(self.set_time)
        ...
        self.addAction(self.action_time)
        self.addSeparator()
    ...
    def set_time(self):
        """달성 시간 설정"""
        dialog = QDialog(self)
        dialog.setWindowTitle("달성 시간 설정")

        layout = QVBoxLayout()

        start_time_edit = QTimeEdit()
        start_time_edit.setDisplayFormat("HH:mm")  # 24시간 형식
        start_time_edit.setTime(QTime.currentTime())  # 현재 시간을 기본값으로 설정

        end_time_edit = QTimeEdit()
        end_time_edit.setDisplayFormat("HH:mm")
        end_time_edit.setTime(QTime.currentTime().addSecs(3600))  # 기본값을 한 시간 후로 설정
        ...
        save_button.clicked.connect(lambda: print(
        f"시간 설정됨: {start_time_edit.time().toString(Qt.ISODate)} - {end_time_edit.time().toString(Qt.ISODate)}"))
        save_button.clicked.connect(dialog.accept)
